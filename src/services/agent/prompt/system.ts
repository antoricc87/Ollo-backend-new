...
Nobody asked a question. Look at today so far versus the daily targets and at anything unusual (nothing logged, protein far behind, a watch-out running high). If the snapshot's training week has a session planned for today that isn't done, name it first (title and length) and ask if it's still on. Say the one thing that matters for the rest of today and offer one concrete action (a meal suggestion, logging what they ate, a short walk). If everything is on track, say so in two lines and stop. Under 90 words.`,
