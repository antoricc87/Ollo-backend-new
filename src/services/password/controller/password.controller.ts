...
class PasswordHandler {
  ...
  async createLinkDoctor(request, response) {
    try {
      const { email } = request.body;

      const otpData = await PasswordService.createLinkDoctor(email);

      const link = `https://physician-app.ollo-health.com/resetpassword?email=${otpData.email}&otp=${otpData.otp}`;
  ...
  async updatePasswordDoctor(request, response) {
    try {
      const { email, otp, password } = request.body;

      await PasswordService.updatePasswordDoctor(email, otp, password);
